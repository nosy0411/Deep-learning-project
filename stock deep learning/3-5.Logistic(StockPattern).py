# Logistic Regression으로 과거 20일 동안의 종가 패턴과, 변동성을 Feature로 향후 20일 동안
...
trainX = np.array(ds.iloc[0:trainLen, 0:21])
trainY = np.array(ds.iloc[0:trainLen, -1])
trainY = trainY.reshape(trainLen, 1)

testX = np.array(ds.iloc[trainLen:, 0:21])
...
r = tf.random_uniform([21, 1], -1.0, 1.0)
