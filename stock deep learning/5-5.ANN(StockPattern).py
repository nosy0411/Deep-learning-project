# 인공신경망을 이용하여 주가의 과거 패턴을 학습하여 목표 수익률을 달성 여부를 학습한다. (이진 분류)
#
...
nInput = 21         # input layer의 neuron 개수
nHidden1 = 50      # hidden layer-1의 neuron 개수
nHidden2 = 50      # hidden layer-2의 neuron 개수
nOutput = 1         # output layer의 neuron 개수
saveDir = './Saver/5-4.save'   # 학습 결과를 저정할 폴더
...
# 데이터 세트를 생성하고, ./dataset/3-5.patternDataset.csv 에 저장해 둔다.
# 시간이 오래 걸린다.
def createDataSet(stockList=stocks, u=0.5, d=-0.5, nPast=20, nHop=1, nFuture=20, binary=True):
    n = 1
    ...
    ft.to_csv('dataset/3-5.patternDataset.csv')

# 데이터 세트를 읽어온다
ds = pd.read_csv('dataset/3-5.patternDataset.csv')
